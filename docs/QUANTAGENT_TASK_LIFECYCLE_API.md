...
这是单机、单进程、单用户、固定离线夹具的工作线程，不是生产级持久队列。服务关闭时会请求正在管理的任务协作停止，并停止接收新的队列工作；进程重启不自动恢复。多个服务实例不得共享同一个运行根目录；没有跨进程锁、强制杀停插件、墙钟预算强制执行、通用重试、外部数据源、OpenStock 写界面、多用户授权或交易权限。取消只在插件步骤之间生效，不能撤销已完成步骤的副作用。真实长任务、恢复和预算能力需要独立设计与验收。
