...
新提交在服务端完成后返回 `201` 和脱敏的运行摘要，`Location` 指向 `GET /api/v1/runs/{run_id}`；相同幂等键及完全相同的正文返回已有运行摘要（`200`），不会重复执行。同一服务实例内，首个提交尚在运行时，同键重试会等待它结束；已完成任务的重试可直接返回，不必等待其他任务。相同键配不同正文返回 `409 idempotency_conflict`。本端点的成功及受控错误响应使用 `Cache-Control: no-store`，受控错误不返回 token、路径或原始证据。`run_id` 仍不是访问凭证。
...
- 同键并发协调仅在单个服务实例内有效；多个进程或实例共用运行目录没有跨进程锁，不属于此 v1 部署模式。
