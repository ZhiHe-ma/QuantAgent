...
| 本机任务生命周期 API v2 | 固定夹具 → 有界单进程队列 → 状态/取消 → v1 已完成结果 | 版本化状态、Bearer、排队/运行中取消、同键重试、冲突、容量限制、重启后 `interrupted`；不宣称跨进程持久恢复或插件内强停 |
| 产品接入（独立 OpenStock fork） | 单账户 → 固定离线任务提交/状态/取消 → 既有结果读取 | 草稿 PR [#2](https://github.com/ZhiHe-ma/OpenStock/pull/2)：与远端 `96a23c1` 同树的本地 `2c32957` 在 Windows 上离线 Vitest 115 通过、6 跳过，范围内 ESLint 0 错误；远端 `verify` 通过。此项不等于后端仓库测试、分支整合或部署验收 |
...
Windows、`PYTHONUTF8=1` 下完整运行 126 个测试：123 个通过、0 个失败、3 个跳过。行为测试覆盖显式启用与默认只读隔离、严格请求 Schema、Bearer、登记夹具与运行根目录隔离、哈希/请求 ID 校验、同键重试及冲突、并发至多一次执行、运行摘要和报告读取，以及本机监听和 token 启动闸门。新增确定性回归覆盖首个同键提交尚未写入状态时的重试等待，以及已完成任务重试不被无关运行阻塞。另有回归检查拒绝 Windows junction 运行根目录；符号链接用例在本机因缺少创建权限跳过，交由 Linux CI 执行。其余两个跳过项仍是未配置专用解释器的真实 Qlib 与 bt 集成测试；这不验证任务队列、取消、强制墙钟预算、OpenStock 写界面或生产部署。
...
## P3e 本机任务生命周期 API v2 本地回归（2026-09-23）

Windows、`PYTHONUTF8=1` 下完整运行 138 个测试：136 个通过、0 个失败、2 个跳过。新增 12 个行为测试覆盖版本化状态 Schema、异步提交与既有结果读取、运行中/排队取消、同键并发去重与冲突、身份验证、非法输入不落盘、已占用运行目录拒绝、容量上限、失败信息脱敏、损坏任务记录失败关闭，以及重启后的保守 `interrupted` 状态。两个跳过项仍是未配置专用解释器的真实 Qlib 与 bt 集成测试；不证明多进程恢复、插件内部强停或生产安全。

PR [#17](https://github.com/ZhiHe-ma/QuantAgent/pull/17) 最终提交 `570d2a4` 的另一次 Windows 完整离线回归记录为 142 项：139 通过、3 跳过、0 失败；第三个跳过项是本机无符号链接创建权限。该 PR 的 Windows 与 Ubuntu GitHub Actions 检查通过，并以 `30cf6fa` 合入 `main`。这与上一段的 138 项属于不同快照，不能相加或写成同一次执行。OpenStock fork 的草稿 PR #2 是独立产品仓库的受控页面增量，尚未完成分支整合或部署验收。
...
未覆盖：真实 DeepSeek 外部联调、在线数据源、真实 thesis 数据适配与来源质量判断、Qlib 供应商数据、Qlib 模型训练、真实行情回测语义、模型驱动 Agent、动态工具调用/墙钟/费用限制、人工暂停恢复、Tool Broker、Typed Handoff、OpenStock 受控任务页面的分支整合与目标环境部署、任意用户研究输入、多用户资源归属/TLS/速率限制、MLflow、Pandera、容器/操作系统级隔离、多来源实盘对账、模型收益评价和实盘交易。
