...
配方是顺序执行的 JSON 文档。当前运行器只要求 `recipe_id`、`recipe_version`、可选说明和非空 `steps`；P0 新增的 Skill、预算与审查字段先冻结语义，P1 实现验证后才可作为执行依据。
...
以下字段在 P0 仅定义规范；当前 `RecipeRunner` 不执行其语义，因此不得据此声称预算、审查或 Skill 绑定已经生效。
