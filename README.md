...
- **Daily**：读取跨日新闻池中最近 24 小时的因子和历史摘要，获取行情并生成日报，然后依次保存日报、尝试推送、生成并保存记忆摘要，最后记录信号审计。

Daily 本身不负责 RSS 新闻采集。首次直接运行 Daily 时，新闻池可能为空；如需包含新闻，应先让 Monitor 完成采集。24 小时窗口锚定日报开始时间，优先按新闻发布时间筛选；发布时间缺失或不可解析时，使用采集时间 `observed_at`，旧缓冲则兼容文件日期加 `time`。无可用时间的记录和未来记录不进入推理；跨日重复记录会去重。采集时间只能说明何时看到新闻，不能证明新闻本身的新鲜度。

Monitor 在缓冲、隔离记录和去重状态写入成功后才确认处理完成。写入异常时保留待写批次，下轮先补写，再抓取 RSS；这能恢复同一进程内的临时写入失败。待写批次位于内存中，不提供进程崩溃时的持久队列保证。

DeepSeek 响应只有 `finish_reason=stop` 且正文有效时才被接受；记忆摘要的核心论点、失效条件、次日检查及观察项均校验文本类型和非空约束。入选新闻的标识、指纹、链接、发布/采集时间与评级校准信息会随因子进入审计记录，便于关联原始来源。
